...
int WAVFile::read_wav_file(std::ifstream& file)
{
	...
	//Prepare buffer
	this->data.init_buffer(data_size / frame_size, sample_rate);

	//Transfer data to buffer
	for (long i = 0; i < (long)(data_size / frame_size); i++)
	{
		this->data[i] = read_word<short>(file, true);
	}
...
int WAVFile::write_wav_file(std::ofstream& file)
{
	...
	for (long i = 0; i < this->data.get_size(); i++)
		write_word(file, (short)((this->data)[i]), this->header.frame_size);
...
void WAVFile::set_buffer(buffer<short>& data)
{
	...
	this->data.init_buffer(data.get_size(), data.get_sample_rate());
