...
namespace DSP
{
	...
	template <typename T>
	void downsample_buffer(const buffer<T>& inbuffer, buffer<T>& outbuffer, int N, int type)
	{
		//Get buffer size
		long size = outbuffer.get_size();

		//Type depending behaviour
		//type = 0 -> just take every Nth sample
		//type = 1 -> take N samples and calculate average
		if (type == 0)
		{
		...
		else if (type == 1)
		{
			for (long i = 0; i < size * N; i += N)
				for (int j = 0; j < N; j++)
					outbuffer[i / N] += (T)((double)inbuffer[i + j] / N);
		}
	}
