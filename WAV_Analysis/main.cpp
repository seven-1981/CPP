...
int main()
{
	//WAV FILE READ
	WAVFile wavfile;
	std::ifstream read_wav("mdh7_1.wav", std::ios_base::in | std::ios_base::binary);
	...
	long pow2samples = pow2_size(window_buffer);
	...
	double freqres = (double)freq_domain.get_sample_rate() / freq_domain.get_size();
	...
	lowcut_freq(freq_domain, freq_filt, 250.0);
	...
	std::ofstream filtwav("filtwav.wav", std::ios_base::out | std::ios_base::binary);
